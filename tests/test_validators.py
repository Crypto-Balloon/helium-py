...
def test_validators():
    ...
    assert 'hash' in next(validators.get_validator_activity(low_seaweed_mandrill))
    ...
    assert 'amount' in next(validators.get_validator_rewards(
        low_seaweed_mandrill, min_time=datetime.now() - timedelta(days=1)))
    assert 'avg' in validators.get_validator_rewards_total(
        low_seaweed_mandrill, min_time=datetime.now() - timedelta(days=1))['data']
