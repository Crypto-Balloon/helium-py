...
from helium_py.api import HELIUM_API_DEFAULT_HOST, ChainVariables

tx_instance = ChainVariables()
base_path = ChainVariables.base_path
...
@mock.patch.object(Session, 'get')
def test_get_by_name(mock_get):
    """Test that client allows custom port."""
    mock_response = mock.Mock(spec=Response)
    mock_response.json.return_value = {'data': [{'some': 'data'}, {'some': 'data'}]}
    mock_get.return_value = mock_response
    response = tx_instance.get_by_name('some_name')
    assert response[0] == {'some': 'data'}
    mock_get.assert_called_once_with(f'https://{HELIUM_API_DEFAULT_HOST}:443/v1/{base_path}/some_name/', params={})
