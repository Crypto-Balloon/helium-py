...
def test_hotspots():
    ...
    assert 'block_added' in hotspots.hotspot_for_address(fresh_cinnamon_duck)['data']
    assert 'block_added' in hotspots.hotspots_for_name('Fresh Cinnamon Duck')['data'][0]
    assert 'block_added' in hotspots.hotspots_search_by_name('Fresh Cinnamon Du')['data'][0]
    ...
    assert 'avg' in hotspots.get_hotspot_rewards_total(
        fresh_cinnamon_duck, min_time=datetime.now() - timedelta(days=1))['data']
    assert 'block_added' in hotspots.get_hotspot_witnesses(fresh_cinnamon_duck)['data'][0]
    assert 'block_added' in hotspots.get_hotspot_witnessed(fresh_cinnamon_duck)['data'][0]
